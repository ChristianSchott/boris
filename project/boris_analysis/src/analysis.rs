...
use hir::{ChangeWithProcMacros, DefMap, HirDisplay, InRealFile, TypeRef};
use ide::Cancelled;
use ide_db::base_db::{SourceDatabase, SourceDatabaseExt, Upcast};
...
impl Analysis {
    ...
    //     let nodes = self.db.file_structure(target_file_id).ok()?;
    //     let offset = nodes
    //         .iter()
    //         .filter(|&node| {
    //             matches!(
    ...
    //     let sema = &Semantics::new(self.db.raw_database());
    ...
    pub fn reload(&mut self, fn_def_id: FunctionId) {
        if let Some(node) = self.function_to_ast(fn_def_id) {
            ...
            if let Some(contents) = path.as_path().and_then(|path| std::fs::read(path).ok()) {
                self.vfs.set_file_contents(path.clone(), Some(contents));
            }
        }
        let mut analysis_change = ChangeWithProcMacros::new();
        for file in self.vfs.take_changes() {
            let contents = SourceDatabaseExt::file_text(&self.db, file.file_id);
            analysis_change.change_file(file.file_id, Some(contents.to_string()))
        }
